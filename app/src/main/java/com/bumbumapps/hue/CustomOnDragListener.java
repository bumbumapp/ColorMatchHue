...
public class CustomOnDragListener implements AdapterView.OnDragListener {
    ...
    // MARK: CONSTRUCTOR
    public CustomOnDragListener(GameActivity gameActivity, int oldPos, int newPos, GridView gridView, int rowlength) {
        this.oldPos = oldPos;
        this.newPos = newPos;
            ...
            case DragEvent.ACTION_DRAG_ENDED:
                ...
                if (dragEvent.getResult()) {
                    ...
                    if (tileAdapter.isPuzzleSolved()) {
                        ...
                        if (rowlength==5){
                            preferenceCoin.putInteger("coin",preferenceCoin.getInteger("coin")+20);
                        }
                        if (rowlength==7){
                            preferenceCoin.putInteger("coin",preferenceCoin.getInteger("coin")+40);
                        }
    ...
    private void showFinishedGamePrompt(View view) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            builder = new AlertDialog.Builder(view.getContext(),R.style.MyDialogStyle);
        } else {
            builder = new AlertDialog.Builder(view.getContext(),R.style.MyDialogStyle);
        }
        builder.setTitle("")
